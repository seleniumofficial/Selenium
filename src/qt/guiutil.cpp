...
static const std::map<ThemedColor, QColor> themedColors = {
    ...
    { ThemedColor::BLUE, QColor(118, 62, 152) },
...
static const std::map<ThemedStyle, QString> themedStyles = {
    ...
    { ThemedStyle::TS_COMMAND, "color:#763E98;" },
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent, bool fAllowURI)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a SELENIUM address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no selenium: URI
    if(!uri.isValid() || uri.scheme() != QString("selenium"))
        return false;
...
{
    QString ret = QString("selenium:%1").arg(info.address);
    int paramCount = 0;
...
void openConfigfile()
{
    ...
    /* Open selenium.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "selenium.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "selenium (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("selenium (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for "selenium*.lnk"
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "selenium.desktop";
    return GetAutostartDir() / strprintf("selenium-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a selenium.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=selenium\n";
        else
            optionFile << strprintf("Name=selenium (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the selenium app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add selenium app to startup item list
