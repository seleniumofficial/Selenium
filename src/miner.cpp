...
// Copyright (c) 2014-2021 The Selenium Core developers
...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(std::shared_ptr<CWallet> &wallet, const CScript &scriptPubKeyIn, bool fProofOfStake)
{
    ...
    if (nHeight > chainparams.GetConsensus().nLastPoWBlock) {
        fProofOfStake = true;
    }
...
{
    LogPrintf("SELENIUMminer -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("SELENIUM-miner");
    ...
    try {
        ...
        while (!fProofOfStake) {
            ...
            std::unique_ptr<CBlockTemplate> pblocktemplate = assembler.CreateNewBlock(coinbaseScript->reserveScript, false);
            if (!pblocktemplate.get())
            {
                LogPrintf("SeleniumMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            auto pblock = std::make_shared<CBlock>(pblocktemplate->block);
            IncrementExtraNonce(pblock.get(), pindexPrev, nExtraNonce);
            LogPrintf("SeleniumMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
            ...
            while (true)
            {
                ...
                if (UpdateTime(&blockHeader, chainparams.GetConsensus(), pindexPrev) < 0)
    ...
    {
        LogPrintf("SeleniumMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("SeleniumMiner -- runtime error: %s\n", e.what());
        return;
    }
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        std::shared_ptr<CWallet> walletPtr(pwallet);
        BitcoinMiner(Params(), connman, walletPtr, false);
        boost::this_thread::interruption_point();
    } catch (const std::exception& e) {
        LogPrintf("SeleniumMiner exception");
    } catch (...) {
        LogPrintf("SeleniumMiner exception");
    }

    LogPrintf("SeleniumMiner exiting\n");
}
