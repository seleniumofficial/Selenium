...
#if defined(NDEBUG)
# error "selenium cannot be compiled without assertions."
#endif
...
{
    const int nLast = Params().GetConsensus().nLastPoWBlock;

    CAmount BlockSubsidy = GetBlockSubsidy(0, nHeight, Params().GetConsensus(), false);

    if (nHeight <= nLast) {
        return static_cast<CAmount>(BlockSubsidy * 0.00);
    } else {
        return static_cast<CAmount>(BlockSubsidy * 0.99);
    }
}
...
void ThreadScriptCheck() {
    RenameThread("selenium-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (llmq::RejectConflictingBlocks()) {
        ...
        for (const auto& tx : block.vtx) {
            ...
            if (llmq::chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                ...
                LOCK(cs_main);
                return state.DoS(10, error("ConnectBlock(SELENIUM): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), conflictLock->txid.ToString()),
                                 REJECT_INVALID, "conflict-tx-lock");
            }
        }
    } else if (!fReindex && !fImporting) {
        LogPrintf("ConnectBlock(SELENIUM): spork is off, skipping transaction locking checks\n");
    }
    ...
    if (pindex->nHeight >= chainparams.GetConsensus().DIP0003EnforcementHeight)
    {
        if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
            return state.DoS(0, error("ConnectBlock(SELENIUM): %s", strError), REJECT_INVALID, "bad-cb-amount");
        }
        ...
        if (!IsBlockPayeeValid(*block.vtx[1], pindex->nHeight, blockReward)) {
            return state.DoS(0, error("ConnectBlock(SELENIUM): couldn't find masternode or superblock payments"),
