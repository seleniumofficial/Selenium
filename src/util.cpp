...
const char * const BITCOIN_CONF_FILENAME = "selenium.conf";
const char * const BITCOIN_PID_FILENAME = "seleniumd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "seleniumd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\selenium
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\selenium
    // Mac: ~/Library/Application Support/selenium
    // Unix: ~/.selenium
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "selenium";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/selenium";
#else
    // Unix
    return pathRet / ".selenium";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty selenium.conf if it does not exist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure selenium copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("selenium") == std::string::npos) {
