...
UniValue setgenerate(const JSONRPCRequest& request)
{
    ...
    if (fGenerate && nHeight > Params().GetConsensus().nLastPoWBlock)
    ...
    GenerateBitcoins(fGenerate, pwallet.get(), nGenProcLimit);
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "selenium is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "selenium is downloading blocks...");

    // next bock is a superblock and we need governance info to correctly construct it
    if (AreSuperblocksEnabled()
        && !masternodeSync.IsSynced()
        && CSuperblock::IsValidBlockHeight(chainActive.Height() + 1))
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "selenium is syncing with network...");
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    { "generating",         "setgenerate",            &setgenerate,            {"nblocks","maxtries"} },
#else
    ...
    { "hidden",             "setgenerate",            &setgenerate,            {"nblocks","maxtries"} },
#endif // ENABLE_MINER
