 ...
 *
 * This is the developer documentation of the reference client for an experimental new digital currency called Dash (https://www.selenium.org/),
...
bool AppInit(int argc, char* argv[])
{
    ...
    if (gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") ||  gArgs.IsArgSet("-help") || gArgs.IsArgSet("-version"))
    {
        ...
        {
            strUsage += "\nUsage:\n"
                  "  seleniumd [options]                     " + strprintf("Start %s Daemon", PACKAGE_NAME) + "\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                fprintf(stderr, "Error: Command line contains unexpected token '%s', see seleniumd -h for a list of options.\n", argv[i]);
                return false;
            }
        }

        // -server defaults to true for seleniumd but not for the GUI so do this here
...
#endif
            fprintf(stdout, "selenium server starting\n");
...
int main(int argc, char* argv[])
{
    ...
    // Connect seleniumd signal handlers
    noui_connect();
