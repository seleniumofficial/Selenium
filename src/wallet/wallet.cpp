...
DBErrors CWallet::ReorderTransactions()
{
    ...
    for (TxItems::iterator it = txByTime.begin(); it != txByTime.end(); ++it)
    {
        ...
        int64_t& nOrderPos = (pwtx != nullptr) ? pwtx->nOrderPos : seleniumentry->nOrderPos;
        ...
        if (nOrderPos == -1)
        {
            ...
            if (pwtx)
            {
                ...
                if (!batch.WriteAccountingEntry(seleniumentry->nEntryNo, *seleniumentry))
                    return DBErrors::LOAD_FAIL;
        }
        else
        {
    ...
    for (auto pcoin : GetSpendableTXs()) {
        ...
        for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
            ...
            if (nCoinType == CoinType::ONLY_FULLY_MIXED) {
                ...
                found = pcoin->tx->vout[i].nValue == Params().GetConsensus().GetMNCollateral(nHeight);
    ...
    for (const auto& outpoint : setWalletUTXO) {
        ...
        for (unsigned int i = 0; i < wtx.tx->vout.size(); i++) {
            ...
            if(fAnonymizable) {
                ...
                if(fMasternodeMode && wtx.tx->vout[i].nValue == Params().GetConsensus().GetMNCollateral(nHeight)) continue;
        ...
        if (const CBlockIndex* pindex = LookupBlockIndex(wtx.hashBlock)) {
            ...
            for (auto it = txOrdered.rbegin(); it != txOrdered.rend(); ++it) {
                ...
                if (pwtx) {
                    ...
                    nSmartTime = seleniumentry->nTime;
                }
...
bool CWallet::SelectStakeCoins(StakeCoinsSet& setCoins, CAmount nTargetAmount, const CScript& scriptFilterPubKey) const
{
    ...
    for (const auto& out : vCoins) {
        ...
        if (GetTime() - out.tx->GetTxTime() < Params().GetConsensus().nStakeMinAge)
            continue;
        if (out.nDepth < (out.tx->tx->IsCoinStake() ? 60 : 10))
            continue;
        if (out.tx->tx->vout[out.i].nValue == Params().GetConsensus().GetMNCollateral(nHeight))
            continue;
