...
bool CheckProRegTx(const CTransaction& tx, const CBlockIndex* pindexPrev, CValidationState& state, const CCoinsViewCache& view)
{
    ...
    if (!ptx.collateralOutpoint.hash.IsNull()) {
        Coin coin;
        if (!view.GetCoin(ptx.collateralOutpoint, coin) || coin.IsSpent() || coin.out.nValue != Params().GetConsensus().GetMNCollateral(pindexPrev->nHeight)) {
        ...
        if (tx.vout[ptx.collateralOutpoint.n].nValue != Params().GetConsensus().GetMNCollateral(pindexPrev->nHeight)) {
            return state.DoS(10, false, REJECT_INVALID, "bad-protx-collateral");
        }
